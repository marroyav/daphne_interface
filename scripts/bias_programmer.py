...
def main(mode,ip_address):
    ...
    if ip_address=="ALL": your_ips = [4,5,7,10,9,11,12,13]; print("No IP specified, reading all endpoints")
    else: your_ips = list(map(int, list(ip_address.split(","))))

    for ip in your_ips:
        if ip not in [4,5,7,10,9,11,12,13]:
        ...
        for vdx,v_value in enumerate(v_used[ip]):
            ...
            print(f'WR BIASSET AFE {vdx} V {v_value}')
